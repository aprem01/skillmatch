 ...
 * scripts/seed-taxonomy-graph.ts (in the PayRanker repo). Returns []
 * when the skill isn't in the graph yet, which is the cue for callers
 * to fall back to the synchronous lib/taxonomy.ts lookup
 * (getSkillIndustries).
 *
 * Caroline 5/22: the matching DB she sketched is a single shared store.
 * Both PayRanker and Skilmatch point at the same Neo4j Aura instance,
 * so this function is the canonical industry source at runtime; the TS
